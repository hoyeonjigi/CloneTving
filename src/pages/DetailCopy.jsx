...
function Detail() {
  ...
  useEffect(() => {
    ...
    const reviewData = async () => {
      try {
        ...
        setReview(JSON.parse(storedReviews)); // 가져온 데이터로 상태를 업데이트합니다.
  ...
  useEffect(() => {
    ...
    const fetchReviews = () => {
      ...
      const storedReviewLength = localStorage.getItem("numberOfReviews");

      if (storedRating && storedReviewLength) {
        setAverageRating(JSON.parse(storedRating));
        setNumberOfReviews(JSON.parse(storedReviewLength));
        return;
      }
      ...
      if (isDataLoaded) {
        ...
        localStorage.setItem("numberOfReviews", JSON.stringify(review.length));
  ...
  }, [isDataLoaded]); // 데이터 로딩 상태를 의존성으로 추가
